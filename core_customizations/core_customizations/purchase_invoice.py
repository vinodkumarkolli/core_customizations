...
def validate_3pl_items(doc, method):
	...
	item_data = frappe.get_all(
		"Item", filters={"name": ("in", item_codes)}, fields=["name", "custom_3pl_item"]
	)
	...
	if doc.update_stock:
		frappe.throw(
			"3PL Items cannot update stock directly from a Purchase Invoice. Please use a Purchase Receipt."
		)
