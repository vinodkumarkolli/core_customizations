...
from core_customizations.monkey_patches import (
	custom_update_stock,
	custom_validate_delivery_note,
	custom_validate_items,
)
...
class TestMonkeyPatches(FrappeTestCase):
	...
	def setUp(self):
		...
		self.warehouse = (
			frappe.db.get_value("Warehouse", {"company": self.company, "is_group": 0}, "name")
			or "Stores - SE-K"
		)
		...
		dn = frappe.get_doc(
			{
				...
				"items": [{"item_code": self.item_code, "qty": 50, "rate": 100, "warehouse": self.warehouse}],
			}
		)
	...
	def test_01_update_stock_context_delivery_note(self):
		...
		doc = frappe._dict({"name": "DN-TEST-001", "selling_price_list": "Standard Selling"})
	...
	def test_03_custom_validate_delivery_note_on_packing_slip(self):
		...
		ps_mock = frappe.get_doc(
			{"doctype": "Packing Slip", "delivery_note": dn.name, "from_package_no": 1, "to_package_no": 1}
		)
		...
		ps_empty = frappe.get_doc(
			{"doctype": "Packing Slip", "delivery_note": None, "from_package_no": 1, "to_package_no": 1}
		)
		with self.assertRaises(frappe.ValidationError):
		...
		ps_valid = frappe.get_doc(
			{
				...
				"from_package_no": 1,
				"to_package_no": 1,
				"items": [{"dn_detail": valid_dn_item, "item_code": self.item_code, "qty": 10}],
			}
		)
		...
		ps_invalid = frappe.get_doc(
			{
				...
				"from_package_no": 1,
				"to_package_no": 1,
				"items": [{"dn_detail": "INVALID-DN-ROW-ID", "item_code": self.item_code, "qty": 10}],
			}
		)
		with self.assertRaises(frappe.ValidationError):
