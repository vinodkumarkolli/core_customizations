...
class TestDeliveryNotePrintFormat(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		if not frappe.db.exists("Supplier", {"supplier_name": cls.transporter_name}):
			...
			cls.transporter_supplier = frappe.db.get_value("Supplier", {"supplier_name": cls.transporter_name}, "name")
		...
		for b_id, itm in [("BATCH-BULB-001", cls.item_code_1), ("BATCH-A", cls.item_code_1), ("BATCH-B", cls.item_code_2)]:
	...
	def _create_test_dn(self, items=None, with_transporter=True):
		...
		dn = frappe.get_doc({
			...
			"custom_transporter_from_address_display": "100 Transporter Origin Hub Street, Chennai, Tamil Nadu" if with_transporter else None,
			...
			"custom_transporter_to_address_display": "200 Destination Godown Road, Bengaluru, Karnataka" if with_transporter else None,
	...
	def test_01_delivery_note_print_formats_exist(self):
		...
		for pf_name in self.print_formats:
			self.assertTrue(
				frappe.db.exists("Print Format", pf_name),
				f"Print Format '{pf_name}' does not exist in DB"
			)
	...
	def test_05_draft_packing_slips_ignored_in_delivery_note_print(self):
		...
		self.assertNotIn("Box #99", html, "Draft Packing Slip Box #99 should NOT appear on Delivery Note print format")
	...
	def test_08_dual_stamp_signature_boxes_and_audit_footer(self):
		...
		self.assertIn("[ Authorised Signatory & Stamp ]", html, "Authorised Signatory Stamp placeholder missing")
	...
	def test_09_company_shipping_address_rendered_in_consignor_column(self):
		...
		self.assertIn("100 Transporter Origin Hub Street", html, "Company dispatch/shipping address should be rendered in Consignor column")
	...
	def test_10_warehouse_address_rendered_in_consignor_column(self):
		...
		html = frappe.get_print("Delivery Note", dn.name, print_format="Delivery Note - Original for Consignee", no_letterhead=1)
		self.assertIn("6/57, MALAI Samy Kovil Street, Madukkarai", html, "Warehouse address line 1 missing in Consignor column")
