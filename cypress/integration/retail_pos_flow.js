context("Retail POS Flow (POS -> SI)", () => {
	...
	before(() => {
		...
		cy.window()
			...
			.then((frappe) => {
				return frappe
					.xcall(
						"core_customizations.tests.test_ui_retail_helpers.setup_cypress_retail_data"
					)
					.then((data) => {
						pos_data = data;
					});
	...
	it("creates a POS Invoice", () => {
		...
		cy.get("div[data-fieldname='item_code'] input")
			.clear()
			.type(`${pos_data.item_code}{enter}`);
	...
	it("creates a POS Closing Entry which generates a Consolidated Sales Invoice", () => {
		...
		cy.get("div[data-fieldname='pos_profile'] input")
			.clear()
			.type(`${pos_data.pos_profile}{enter}`);
		cy.wait(500);
		...
		cy.get("div[data-fieldname='payment_reconciliation'] .grid-row")
			.first()
			.within(() => {
				cy.get("input[data-fieldname='expected_amount']")
					.invoke("val")
					.then((val) => {
						cy.get("input[data-fieldname='closing_amount']")
							.clear()
							.type(`${val}{enter}`);
					});
			});
